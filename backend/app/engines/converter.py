...
# Respect configuration flag to allow disabling WeasyPrint at runtime
try:
    from app.core.config import settings
except Exception:
    settings = None

if not (getattr(settings, "DISABLE_WEASYPRINT", False) if settings is not None else False):
    try:
        from weasyprint import HTML, CSS
        WEASYPRINT_AVAILABLE = True
    except Exception:
        # WeasyPrint may raise OSError when native libs are missing
        # catch broad exceptions to avoid crashing application at import time
        WEASYPRINT_AVAILABLE = False
else:
    WEASYPRINT_AVAILABLE = False
...
class DocumentConverter:
    ...
    def html_to_pdf(html_path: Path, output_path: Path, css_path: Optional[Path] = None) -> Path:
        ...
        if not WEASYPRINT_AVAILABLE:
            raise RuntimeError(
                "WeasyPrint is not installed. Please install weasyprint package.")
    ...
    def markdown_to_pdf(md_path: Path, output_path: Path) -> Path:
        ...
        if not MARKDOWN_AVAILABLE:
            raise RuntimeError(
                "Markdown is not installed. Please install markdown package.")
        if not WEASYPRINT_AVAILABLE:
            raise RuntimeError(
                "WeasyPrint is not installed. Please install weasyprint package.")
    ...
    def word_to_pdf(docx_path: Path, output_path: Path) -> Path:
        ...
        if not PYTHON_DOCX_AVAILABLE:
            raise RuntimeError(
                "python-docx is not installed. Please install python-docx package.")
        if not WEASYPRINT_AVAILABLE:
            raise RuntimeError(
                "WeasyPrint is not installed. Please install weasyprint package.")
    ...
    def excel_to_pdf(xlsx_path: Path, output_path: Path) -> Path:
        ...
        if not OPENPYXL_AVAILABLE:
            raise RuntimeError(
                "openpyxl is not installed. Please install openpyxl package.")
        if not WEASYPRINT_AVAILABLE:
            raise RuntimeError(
                "WeasyPrint is not installed. Please install weasyprint package.")
        ...
        for sheet_name in wb.sheetnames:
            ...
            sheets_html.append(
                f"<h2>{sheet_name}</h2><table>{''.join(rows_html)}</table>")
    ...
    def ppt_to_pdf(pptx_path: Path, output_path: Path) -> Path:
        ...
        if not PYTHON_PPTX_AVAILABLE:
            raise RuntimeError(
                "python-pptx is not installed. Please install python-pptx package.")
        if not WEASYPRINT_AVAILABLE:
            raise RuntimeError(
                "WeasyPrint is not installed. Please install weasyprint package.")
