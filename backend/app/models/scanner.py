...
class ScanSession(Base):
    ...
    status = Column(
        String(50), default=ScanStatus.PENDING.value, nullable=False)

    # Settings
    scan_mode = Column(
        String(20), default=ScanMode.COLOR.value, nullable=False)
    dpi = Column(Integer, default=300, nullable=False)
    output_format = Column(String(10), default="pdf",
                           nullable=False)  # pdf, png, jpg
    ...
    # Relationships
    pages = relationship("ScanPage", back_populates="session",
                         cascade="all, delete-orphan", order_by="ScanPage.page_number")
    output_document = relationship(
        "Document", foreign_keys=[output_document_id])
...
class ScanPage(Base):
    ...
    session_id = Column(String(36), ForeignKey(
        "scan_sessions.id", ondelete="CASCADE"), nullable=False)
    ...
    corners_detected = Column(JSON, nullable=True)
    ...
    processing_applied = Column(JSON, nullable=True)

    # Metadata
    # camera, webcam, upload
    capture_source = Column(String(50), nullable=True)
